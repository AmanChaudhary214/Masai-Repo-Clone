...
public class ABCVocationalSchool {
	...
	static double getAttendancePercentage(long courseDurationInDays, long medicalLeavesInDays, long attendanceInDays) throws TooShortAttendanceException {
		...
		//long attendance = courseDurationInDays - medicalLeavesInDays;
		double attPercentage = (Math.round(((double)attendanceInDays/(double)(courseDurationInDays-medicalLeavesInDays))*100.0));
	...
	static String getRemark(double attendancePercentage){
		...
		else if (attendancePercentage >= 90.0 && attendancePercentage < 95.0) {
			return "You got 8 out of 10 marks in discipline and extracurricular activity";
		}
		else if (attendancePercentage >= 85.0 && attendancePercentage < 90.0) {
			return "You got 6 out of 10 marks in discipline and extracurricular activity";
		}
		else if (attendancePercentage >= 80.0 && attendancePercentage < 85.0) {
	...
	public static void main(String args[]){
	    ...
	    try{
	      ...
	      double attendancePercentage = getAttendancePercentage(courseDurationInDays, medicalLeavesInDays, attendanceInDays);
