...
let areasDirectory = areas.reduce((acc, element) => {
  let id = element.id;
...
let outputArray = inputArray.map((element) => {
  ...
  obj['productId'] = element.idMeal;
  obj['productTitle'] = element.strMeal;
  obj['Category'] = categoriesDrectory[element.Category];
  obj['Area'] = areasDirectory[element.Area];
  obj['Ingredients'] = [];

  for (let i=1; i<=20; i++)
  {
    element[`strIngredient${i}`] && obj['Ingredients'].push({ingredient:element[`strIngredient${i}`], measure:element[`strMeasure${i}`]});
  }
  return obj;
});
